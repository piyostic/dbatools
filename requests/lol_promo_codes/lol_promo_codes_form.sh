...
pBASEDIR=$(dirname $0)
pPID=$$
pFORMID="12mMdlNYXL5-xXJqYObsmb1n5aPo7KghInjDruEMd0lk"
...
then
	echo "Timestamp,Username,Region,Bundle ID,Amount,Jira Ticket" > $pBASEDIR/form-processed.csv
fi
...
do        
	...
	pBUNDLE=$(echo $pREQUEST | awk -F "," '{print $4}')
	pAMOUNT=$(echo $pREQUEST | awk -F "," '{print $5}')
	pJIRA=$(echo $pREQUEST | awk -F "," '{print $6}')
	...
	if [ "$pTIMESTAMP" != "Timestamp" ]; then
		...
		if [ "$pPROCESSED" = "" ]; then
			...
			$pBASEDIR/lol_promo_codes.sh $pREGION $pBUNDLE $pAMOUNT $pJIRA $pUSER
