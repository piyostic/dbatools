...
pBASEDIR=$(dirname $0)
pPID=$$
pFORMID="1dlggsA_9TiHLy8lDgjaj-m5yHRGSRVBGvxrEp3xJNX8"
...
then
	echo "Timestamp,Username,Region,Churn Days From,Churn Days To,Summoner Level From,Summoner Level To" > $pBASEDIR/form-processed.csv
fi
...
do        
	...
	pREGION=$(echo $pREQUEST | awk -F "," '{print $3}')
	pCHURNFROM=$(echo $pREQUEST | awk -F "," '{print $4}')
	pCHURNTO=$(echo $pREQUEST | awk -F "," '{print $5}')
	pLEVELFROM=$(echo $pREQUEST | awk -F "," '{print $6}')
	pLEVELTO=$(echo $pREQUEST | awk -F "," '{print $7}')
	...
	if [ "$pTIMESTAMP" != "Timestamp" ]; then
		...
		if [ "$pPROCESSED" = "" ]; then
			...
			$pBASEDIR/lol_churn_level.sh $pREGION $pCHURNFROM $pCHURNTO $pLEVELFROM $pLEVELTO $pUSER
