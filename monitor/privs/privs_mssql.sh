#!/bin/bash
##################################################################################
# Written by Rosalind Chan on 27-Sep-2015
# This script checks mssql privileges and inserts into monitor DB
# e.g. ./privs.sh PB ID
# Parameter 1 : Game ID
...
WHERE a.GAME_ID = '$pGAMEID' AND a.LOCATION = '$pLOCATION' and a.DBTYPE in ('MSSQL') and a.ACTIVE = 1 ORDER BY a.IP;"
...
do
	...
	echo "[$(date +"%F %T")] Collecting Logins"
	pSQL="set nocount on; SELECT $pSERVERID,name,'%',CONVERT(varchar(max), LOGINPROPERTY(name, 'PasswordHash'),1 ),0,null FROM sys.server_principals pr WHERE type IN ('U','S') and name NOT LIKE '#%';"
	sqlcmd -S $pSQLHOST,$pSQLPORT -U $pDBUSER -P $pDBPASS -Q "$pSQL" -W -w 1024 -s"#" | \
	grep -v '^-' -r0 2> $pBASEDIR/err-$pPID.log | tr '#' '\t' | tail -n+2 >> $pBASEDIR/privs-$pPID-$pGAMEID$pLOCATION.tsv
	...
	echo "[$(date +"%F %T")] Collecting Grant definitions"
	sqlcmd -S $pSQLHOST,$pSQLPORT -U $pDBUSER -P $pDBPASS -i $pBASEDIR/privs_mssql.sql -W -w 1024 -s"#" -r0 2> $pBASEDIR/err-$pPID.log | \
	grep -v -e '^name#' -e '^---' -e '^ # #' -e '^-#-#' | tr '#' '\t' | sort | \
	awk -v VAR="$pSERVERID" -F'\t' '{ stuff[$1"@%"] = stuff[$1"@%"] $3 " "} END { for( s in stuff ) print VAR "\t" s "\t" stuff[s] }' >> $pBASEDIR/grants-$pPID-$pGAMEID$pLOCATION.tsv
	if [[ -s $pBASEDIR/err-$pPID.log ]]
        then
                echo "[$(date +"%F %T")] Error Collecting Grant definitions from $pNAME-$pIP:$pPORT" | tee -a $pBASEDIR/err-$pPID.log
                pEXCLUDE="${pEXCLUDE}${pSERVERID},"
        fi
...
#Temp table to store collected logins
pSQL="CREATE TEMPORARY TABLE tmp_privs like privs;"
pSQL="$pSQL LOAD DATA LOCAL INFILE '$pBASEDIR/privs-$pPID-$pGAMEID$pLOCATION.tsv' into table tmp_privs FIELDS ESCAPED BY '' (DBSERVER_ID,USER_NAME,USER_HOST,PASSWORD,MAX_USER_CONNECTIONS,DEFINITION) SET OS_PROCESS_ID=$pPID;"

echo "[$(date +"%F %T")] Loading $pBASEDIR/grants-$pPID-$pGAMEID$pLOCATION.tsv"
#Temp table to store collected grants
pSQL="$pSQL CREATE TEMPORARY TABLE tmp_grants (id integer,user varchar(100),definition text);"
pSQL="$pSQL LOAD DATA LOCAL INFILE '$pBASEDIR/grants-$pPID-$pGAMEID$pLOCATION.tsv' into table tmp_grants FIELDS ESCAPED BY '';"

#Massage temp table data
...
pSQL="$pSQL UPDATE tmp_privs SET password=null where password='NULL';"
