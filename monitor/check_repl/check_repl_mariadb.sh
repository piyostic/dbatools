...
# This script checks multi-source replication slave status
...
a.ID_MASTER is not null and a.DBTYPE = 'MariaDB' and a.ACTIVE=1  
ORDER BY a.IP;"
...
do
	...
	pSQL="SHOW ALL SLAVES STATUS;"
	mysql -Ns -h $pSQLHOST -P $pSQLPORT -u $pDBUSER -p$pDBPASS -e "$pSQL" | \
	while read pDATA2
	do
        	#Read data from table
	        pCONN=$(echo -e "$pDATA2" | awk -F'\t' '{print $1}')

        	pSQLCHECK="SHOW SLAVE '$pCONN' STATUS\G"
	        ...
	        if ! [[ $pSLAVESEC =~ $pREG ]] ; then
	                echo "[$(date +"%F %T")] Replication $pCONN Errors for $pNAME-$pIP:$pPORT" | tee $pBASEDIR/err-$pPID.log
	                ...
	                if [[ $pSLAVESEC -ge $pREPLMAXLAG ]] ; then
        	                echo "[$(date +"%F %T")] Replication $pCONN Lag for $pNAME-$pIP:$pPORT more than $pREPLMAXLAG seconds" | tee $pBASEDIR/err-$pPID.log
	                        cat $pBASEDIR/err-$pPID-$pIP$pPORT.log | tee -a $pBASEDIR/err-$pPID.log
	                else
	                        echo "[$(date +"%F %T")] Replication $pCONN up and $pSLAVESEC second(s) behind master"
	                fi
